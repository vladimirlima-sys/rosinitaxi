      ...
      (pos) => {
        ...
        if (lastPositionRef.current) {
          ...
          if (delta > 0 && delta < 0.5 && acc < 50) {
            ...
            const price = km * getPricePerKm(priceSettings, vehicleType) + (priceSettings?.base_fare || 0);
    ...
    <div className="min-h-screen bg-[#0A0A0A] flex flex-col items-center justify-start px-4 py-10">
      <div className="w-full max-w-sm space-y-5">
        ...
        {!running ? (
          <div className="space-y-2">
            ...
            >
              {!priceSettings ? (
                ...
                <><Play className="w-5 h-5" /> Iniciar Corrida</>
              )}
            </button>
            {status !== 'idle' && (
              <button
          ...
          >
            <Square className="w-5 h-5" /> Parar Corrida
          </button>
        )}
