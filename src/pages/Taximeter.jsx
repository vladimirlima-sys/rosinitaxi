...
export default function Taximeter() {
  ...
  const getPricePerKm = () => {
    if (!priceSettings) return 2.36;
    const standard = priceSettings.taximeter_standard_price_per_km || 2.36;
  ...
  const getBaseFare = () => {
    ...
    return typeof fare === 'number' && fare >= 0 ? fare : 13;
  };
  ...
  useEffect(() => {
    if (running && priceSettings) {
      const waitingPricePerMin = getWaitingPricePerMinute();
      const newWaitingPrice = (waitingSeconds / 60) * waitingPricePerMin;
      ...
      const total = baseFare + kmPrice + nightSurcharge + newWaitingPrice;
  ...
  const startRide = () => {
    ...
    watchIdRef.current = navigator.geolocation.watchPosition(
      (pos) => {
         ...
         // Use raw coordinates directly - simplify by removing HERE call
