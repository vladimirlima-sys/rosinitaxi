...
export default function VehicleCard({ type, selected, onSelect, distance }) {
  ...
  const config = isEconomic ? {
    ...
    features: ['Berline confortable', 'Climatisation', 'Wi-Fi gratuit', 'Bagages inclus']
  } : {
    name: 'Confort',
    capacity: 4,
    pricePerKm: 2.95,
    image: 'https://images.unsplash.com/photo-1563720223185-11003d516935?w=600&q=80',
    features: ['Véhicule premium', 'Sièges cuir', 'Wi-Fi gratuit', 'Eau & rafraîchissements', 'Chargeurs USB']
  };
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-1.5 text-white/50">
          ...
          <span className="text-sm">{config.capacity} pers.</span>
        </div>
      </div>
      ...
      <div className="mb-5">
        ...
        {totalPrice && (
          <p className="text-white/50 text-sm mt-1">
            Estimation: <span className="text-[#C9A96E] font-medium">CHF {totalPrice}</span>
          </p>
        )}
      </div>
