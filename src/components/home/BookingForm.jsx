...
export default function BookingForm({ bookingRef, sharedForm, sharedUpdate, sharedEstimatedDistance, sharedEstimatedTime, sharedPriceSettings }) {
  const { lang } = useLang();
  const t = translations[lang];

  const form = sharedForm;
  const update = sharedUpdate;
  const estimatedDistance = sharedEstimatedDistance || 0;
  const priceSettings = sharedPriceSettings;
  ...
  // Don't show the booking form until the hero has a valid route
  if (estimatedDistance === 0) return null;

  return (
    <section ref={bookingRef} className="py-20 px-6 bg-[#F5C300]">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Steps indicator (steps 2-4 shown as 1-3) */}
        <div className="flex items-center justify-center gap-2 mb-12">
          {[2, 3, 4].map((s, i) => (
            <div key={s} className="flex items-center">
              ...
              {i < 2 && <div className={`w-12 h-[2px] ${step > s ? 'bg-black' : 'bg-black/20'}`} />}
            </div>
          ))}
        </div>
            ...
            <div className="space-y-4 p-6 rounded-2xl bg-white border border-black/10">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <label key={value} className="flex items-center gap-3 cursor-pointer p-4 rounded-xl border-2 transition-all"
                    style={{ borderColor: paymentMethod === value ? '#000' : 'rgba(0,0,0,0.15)', backgroundColor: paymentMethod === value ? 'rgba(0,0,0,0.08)' : 'rgba(255,255,255,0.5)' }}>
                    <input type="radio" name="payment" value={value} checked={paymentMethod === value} onChange={(e) => setPaymentMethod(e.target.value)} className="w-4 h-4" style={{ accentColor: '#000' }} />
                    <span className="text-black font-medium flex-1">{label}</span>
                    <span className="text-black/50 text-sm">{desc}</span>
                  </label>
                ))}
              </div>
            </div>
