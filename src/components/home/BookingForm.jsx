                ...
                <div className="text-right">
                  {priceSettings && estimatedDistance > 0 && (
                    <p className={`font-bold ${form.vehicle_type === 'economic' ? 'text-[#F5C300]' : 'text-black'}`}>
                      CHF {(() => {
                        let p = estimatedDistance * priceSettings.standard_price_per_km + (estimatedDistance <= 5 ? 13 : estimatedDistance <= 50 ? (priceSettings.base_fare || 0) : 0);
                        return p.toFixed(2);
                      })()}
                    </p>
                  )}
                ...
                <div className="text-right">
                  {priceSettings && estimatedDistance > 0 && (
                    <p className={`font-bold ${form.vehicle_type === 'comfort' ? 'text-[#F5C300]' : 'text-black'}`}>
                      CHF {(() => {
                        let p = estimatedDistance * (priceSettings.comfort_price_per_km || priceSettings.standard_price_per_km * 1.3) + (estimatedDistance <= 5 ? 13 : estimatedDistance <= 50 ? (priceSettings.base_fare || 0) : 0);
                        return p.toFixed(2);
                      })()}
                    </p>
                  )}
          ...
          <div className="space-y-2 sm:space-y-3">
            ...
            <div className="bg-black border border-black/40 rounded-xl p-3 sm:p-4 space-y-2">
              ...
              {nightSurchargeAmount > 0 && (
                <div className="flex justify-between text-sm"><span className="text-yellow-400/80">🌙 Supplément nuit ({priceSettings.night_surcharge_percentage}%)</span><span className="text-yellow-400">+CHF {nightSurchargeAmount.toFixed(2)}</span></div>
              )}
              {hasValaisFribourgSurcharge && (
                <div className="flex justify-between text-sm"><span className="text-orange-400/80">Supplément Valais/Fribourg ({priceSettings.valais_fribourg_surcharge_percentage}%)</span><span className="text-orange-400">inclus</span></div>
              )}
