...
export default function BookingForm({ bookingRef }) {
  ...
  const isShortNotice = () => {
    ...
    const departure = new Date(`${form.departure_date}T${form.departure_time}:00`);
    const now = new Date();
      ...
      <div className="w-full max-w-md">
        ...
        {step === 1 && (
          <div className="space-y-3">
            ...
            {isShortNotice() && (
              <div className="bg-black border border-black rounded-xl p-4 flex gap-3 items-start">
                <span className="text-[#F5C300] text-lg mt-0.5">⚠️</span>
                <p className="text-[#F5C300] text-sm leading-relaxed">{t.shortNoticeWarning}</p>
              </div>
            )}
        ...
        {step === 2 && (
          <div className="space-y-3">
            ...
            <div className="flex gap-3">
              <button onClick={() => setStep(1)} className="flex-1 h-12 rounded-xl border-2 border-white bg-[#F5C300] text-black font-bold text-sm uppercase tracking-wider hover:bg-black hover:text-white hover:border-black transition-all">{t.backBtn}</button>
              <button
                onClick={() => setStep(3)}
        ...
        {step === 3 && (
          <div className="space-y-3">
            ...
            <div className="flex gap-3">
              <button
                onClick={() => setStep(2)}
                className="flex-1 h-12 rounded-xl border-2 border-white bg-[#F5C300] text-black font-bold text-sm uppercase tracking-wider hover:bg-black hover:text-white hover:border-black transition-all"
              >
                {t.backBtn}
              </button>
              <button
                ...
                className={`flex-[2] h-12 rounded-xl font-bold text-sm uppercase tracking-wider transition-all ${
                  canProceedStep3
                    ? 'bg-[#F5C300] text-black hover:bg-black hover:text-white'
                    : 'bg-white/10 text-white/40 cursor-not-allowed'
                }`}
              >
                {t.paymentBtn}
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-[2] h-12 rounded-xl border-2 border-white bg-[#F5C300] text-black font-bold text-sm uppercase tracking-wider hover:bg-black hover:text-white hover:border-black transition-all disabled:opacity-60 flex items-center justify-center gap-2"
              >
        ...
        {step === 5 && (
          <div className="text-center py-12 space-y-4">
            ...
            <p className="text-white/60 text-sm max-w-xs mx-auto">{t.confirmMsg(form.client_name, form.departure_point, form.arrival_point)}</p>
            <p className="text-white/40 text-xs">{t.confirmEmail(form.client_email)}</p>
            <button
              onClick={resetForm}
